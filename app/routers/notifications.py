...
async def notification_websocket(
    ...
    try:
        db = next(get_db())
        unread = NotificationService.get_unread_messages(
...
@router.get("/api/v1/notifications", response_model=PaginatedResponse, tags=["【9. 消息通知】"])
...
@router.get("/api/v1/notifications/unread", response_model=ResponseModel, tags=["【9. 消息通知】"])
def get_unread(user_id: Optional[int] = None, role: Optional[str] = None,
               limit: int = 50, db: Session = Depends(get_db)):
    msgs = NotificationService.get_unread_messages(db, user_id=user_id, role=role, limit=limit)
...
@router.put("/api/v1/notifications/{msg_id}/read", response_model=ResponseModel, tags=["【9. 消息通知】"])
...
@router.put("/api/v1/notifications/read-all", response_model=ResponseModel, tags=["【9. 消息通知】"])
def mark_all_read(user_id: Optional[int] = None, db: Session = Depends(get_db)):
    q = db.query(NotificationMessage).filter(NotificationMessage.is_read == False)
    msgs = q.all()
    for m in msgs:
        m.is_read = True
        m.read_at = datetime.utcnow()
    db.commit()
    return ResponseModel(data={"marked_count": len(msgs)})


@router.post("/api/v1/notifications/push", response_model=ResponseModel, tags=["【9. 消息通知】"])
async def manual_push(
    ...
    msg = NotificationService.create_notification(
